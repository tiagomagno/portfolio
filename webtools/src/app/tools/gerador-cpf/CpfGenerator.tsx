...
import { generateCpf } from "../../lib/cpf-cnpj";
...
export default function CpfGenerator() {
  ...
  function generate() { setList(Array.from({ length: qty }, () => generateCpf(formatted))); }
      ...
      <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 10, padding: 16, marginBottom: 16, display: "flex", gap: 16, flexWrap: "wrap", alignItems: "center" }}>
        ...
        <div>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            {[{ v: true, l: "000.000.000-00" }, { v: false, l: "00000000000" }].map(({ v, l }) => (
      ...
      {list.length > 0 && (
        <>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
            <label style={{ fontSize: 12, fontWeight: 600, color: "var(--text-muted)" }}>{list.length} CPF{list.length !== 1 ? "S" : ""} GERADO{list.length !== 1 ? "S" : ""}</label>
          ...
          <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 10, overflow: "hidden" }}>
            {list.map((cpf, i) => (
              <div key={i} style={{ padding: "10px 14px", borderBottom: i < list.length - 1 ? "1px solid var(--border)" : "none", fontFamily: "monospace", fontSize: 15, color: "var(--accent)", display: "flex", justifyContent: "space-between" }}>
                {cpf}
                <button onClick={() => navigator.clipboard.writeText(cpf)}
          ...
          <p style={{ fontSize: 11, color: "var(--text-muted)", marginTop: 8 }}>⚠ CPFs gerados para fins de teste. Não use para fraudes.</p>
        </>
      )}
    </div>
  );

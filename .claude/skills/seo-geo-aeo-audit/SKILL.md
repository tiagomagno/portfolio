...
> "Você quer uma **Auditoria Rápida** (principais problemas e notas — leva 1-2 minutos) ou uma **Auditoria Completa** (análise abrangente em todas as dimensões — leva mais tempo)?"

Wait for the user's reply before doing anything else. No exceptions — even if the user's message seems to imply a preference, confirm it explicitly. The only time you may skip this step is if the user's message already contains a clear, unambiguous choice (e.g. "faz uma auditoria completa de..." ou "auditoria rápida por favor").

---

## Step 2: Fetch and collect data

Use WebFetch (or the Browser tools, when a rendered/JS-heavy view is needed) to gather page data. **Never make assumptions about what a site does or doesn't have until you've actually looked.** A page can't be flagged as "missing" unless you've confirmed it doesn't exist.
...
- **Same-as / brand entity links**: Social profile links pointing from the site (strengthens entity graph)?
...
Immediately after the brief chat recap, generate the full report as a `.docx` (and a `.pdf` if the environment's PDF conversion tooling is available). Do not ask the user if they want this — just produce it, then use `SendUserFile` to deliver it.

**In Claude Code, do not hand-roll a `docx` script or install the `docx` npm package directly.** Invoke the `docx` Skill (via the Skill tool) to build the Word document, and the `pdf` Skill if a PDF copy is also needed — both already handle the underlying tooling correctly for this environment. Write intermediate content to the session scratchpad directory, not the project repo.

Tell the user: "Gerando o relatório para download..."

### Report design

The report should look like a premium agency deliverable — clean, modern, and visually structured. Use this design system as guidance to the `docx` skill:
...
**Typography:** clean sans-serif throughout. Title 36pt bold, H1 24pt bold, H2 18pt bold, H3 14pt bold, body 11pt, footer 9pt.

**Page setup:** US Letter or A4, 1-inch margins on all sides.

### Report structure

Build the report in this order:

1. **Cover page** — site domain as the hero element (large, bold), "SEO / GEO / AEO Audit Report" subtitle, audit type (Quick/Full), a 3-column score summary table (SEO/GEO/AEO, color-coded by score), audit date, attribution footer ("Auditoria gerada por Tiago Magno via Claude").
2. **Executive summary** — a shaded callout box with 3-5 sentences on the site's overall position (strongest point, most urgent issue, one key opportunity), followed by the SEO/GEO/AEO/Combined scores table.
3. **Pages audited** — table of every page fetched: URL | Page Type | Notes.
4. **SEO analysis** — sub-sections: Technical On-Page, Content Quality, Structured Data. For each finding, a 3-column table: Signal | Finding | Status (color-coded Good/Needs Attention/Missing).
5. **GEO analysis** — same structure. Sub-sections: E-E-A-T Assessment, Content for AI Synthesis, Technical GEO.
6. **AEO analysis** — same structure. Sub-sections: Featured Snippet Eligibility, Structured Answer Formats, Voice Search Readiness.
7. **Priority recommendations matrix** — 5 columns: Priority | Issue | Dimension | Effort | Impact. Color-code Priority: 🔴 Critical, 🟠 High, 🟡 Medium, 🟢 Quick Win.
8. **What's working well** — a green-tinted table listing genuine strengths with specific evidence from the crawl.
9. **Glossary** (Full Audit only) — brief plain-English definitions of SEO, GEO, and AEO.

### Deliver to the user

Use `SendUserFile` to send the generated `.docx` (and `.pdf` if produced), with a short caption naming the site and audit type.

---

## Step 6: Invite next steps

> "Quer que eu aprofunde em alguma área específica? Também posso auditar páginas adicionais, comparar este site com o de um concorrente, ou rodar a auditoria de novo depois que você fizer as mudanças."
